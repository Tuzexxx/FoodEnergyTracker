        ...
        <div className="w-full flex flex-col gap-6 animate-in fade-in duration-300">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="min-w-0 flex-1">
                    <h2 className="font-drama text-2xl sm:text-3xl text-brutal-black tracking-tight leading-tight">{t.progress.title}</h2>
                    <p className="font-sans text-xs text-brutal-black/50 tracking-wide mt-0.5">{dateRangeLabel}</p>
                </div>

                {/* 7d / 30d Toggle */}
                <div className="bg-black/5 p-1 rounded-full border border-black/5 flex items-center gap-1 shadow-inner shrink-0 self-start sm:self-auto">
                    <button
                        onClick={() => {
                            setPeriod('7d');
                            setSmartwatchInput(smartwatchWeeklyBurn ? smartwatchWeeklyBurn.toString() : '');
                        }}
                        className={`px-3 sm:px-3.5 py-1 rounded-full font-sans text-xs font-bold uppercase tracking-wider transition-all ${period === '7d' ? 'bg-brutal-black text-off-white shadow-sm' : 'text-brutal-black/50 hover:text-brutal-black'}`}
                    >
                    ...
                    <button
                        ...
                        className={`px-3 sm:px-3.5 py-1 rounded-full font-sans text-xs font-bold uppercase tracking-wider transition-all ${period === '30d' ? 'bg-brutal-black text-off-white shadow-sm' : 'text-brutal-black/50 hover:text-brutal-black'}`}
                    >
            ...
            <div className={`p-6 sm:p-8 rounded-3xl border-2 shadow-xl flex flex-col items-center justify-center text-center relative overflow-hidden transition-all duration-500 ${
                ...
                <div className="flex items-center gap-2 mb-2 bg-white/20 backdrop-blur-md px-3 py-1 rounded-full text-[10px] font-sans font-bold uppercase tracking-wider sm:tracking-widest max-w-full text-center">
                    {telemetry.isDeficit ? <TrendingDown size={14} className="shrink-0" /> : <TrendingUp size={14} className="shrink-0" />}
                    <span className="truncate">{telemetry.isDeficit ? t.progress.deficitBadge : t.progress.surplusBadge}</span>
                </div>
                ...
                <span className="font-sans text-xs uppercase font-bold tracking-wider sm:tracking-widest opacity-90 max-w-full px-2 text-center">
                ...
                <div className="mt-4 pt-3 border-t border-white/20 w-full flex justify-around text-center text-xs">
                    <div className="px-2">
                        ...
                        <strong className="font-data text-sm sm:text-base font-bold">{telemetry.netDeficitKcal > 0 ? `-${telemetry.netDeficitKcal}` : `+${Math.abs(telemetry.netDeficitKcal)}`} {t.common.kcal}</strong>
                    </div>
                    <div className="w-[1px] bg-white/20" />
                    <div className="px-2">
                        <span className="opacity-60 block text-[10px] uppercase font-bold">{t.progress.gymDays}</span>
                        <strong className="font-data text-sm sm:text-base font-bold">{exerciseDaysCount} / {daysCount}</strong>
                    </div>
                </div>
            </div>
            ...
            {optimalFatGrams > 0 && (
                <div className="p-5 rounded-3xl bg-white border-2 border-brutal-black/20 shadow-sm flex flex-col gap-3 animate-in fade-in">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 items-start">
                        <div className="flex items-center gap-2">
                            ...
                            <div className="min-w-0">
                                ...
                                <span className="text-[11px] text-brutal-black/50 font-sans block leading-tight">
                                    {t.progress.optimalPlanDesc}
                                </span>
                            </div>
                        </div>
                        <span className={`text-xs font-data font-bold px-2.5 py-1 rounded-full border shrink-0 self-start sm:self-auto ${
                    ...
                    <div className="flex flex-col gap-1.5 pt-1">
                        <div className="flex flex-wrap justify-between gap-1 text-xs font-sans font-semibold text-brutal-black/80">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full">
                <div className="p-4 rounded-2xl bg-white border border-black/5 shadow-sm flex flex-col justify-between">
                    <span className="font-sans text-[10px] uppercase font-bold text-brutal-black/50 tracking-wider truncate">{t.progress.totalConsumed}</span>
                ...
                <div className="p-4 rounded-2xl bg-white border border-black/5 shadow-sm flex flex-col justify-between">
                    <span className="font-sans text-[10px] uppercase font-bold text-brutal-black/50 tracking-wider truncate">{t.progress.totalBurned}</span>
                    <div className="flex items-baseline gap-1 mt-2">
            ...
            <div className="p-5 rounded-3xl bg-white border border-black/5 shadow-sm flex flex-col gap-3">
                <div className="flex items-center gap-2 text-brutal-black/80 font-sans font-bold text-xs uppercase tracking-wider">
                    <Watch size={16} className="text-orange-500 shrink-0" />
                    <span className="leading-tight">{t.progress.smartwatchTitle} ({period === '7d' ? t.progress.sevenDays : t.progress.thirtyDays})</span>
                </div>
                ...
                <div className="flex items-center gap-2 mt-1">
                    <input
                        ...
                        className="flex-1 min-w-0 bg-black/5 text-brutal-black px-4 py-2.5 rounded-2xl font-data font-bold text-sm outline-none focus:bg-white focus:ring-2 focus:ring-orange-400/50 border border-transparent transition-all"
                    />
                    <button
                        onClick={handleSaveWatch}
                        className="px-4 py-2.5 bg-brutal-black text-off-white hover:bg-brutal-black/90 active:scale-95 transition-all rounded-2xl font-sans font-bold text-xs uppercase tracking-wider shadow-sm shrink-0"
                    >
                        {t.common.apply}
                    </button>
                    {(period === '7d' ? smartwatchWeeklyBurn : smartwatchMonthlyBurn) && (
                        <button
                            ...
                            className="p-2.5 bg-black/5 hover:bg-black/10 text-brutal-black/60 rounded-2xl active:scale-95 transition-all shrink-0"
